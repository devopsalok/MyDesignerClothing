...
package com.mydesignerclothing.mobile.android.commons.core.optional;
 ...
 * {@link #checkNotNull(Object)} or { Verify#verifyNotNull(Object)} is appropriate to the
...
public final class Preconditions {
  ...
  static String format(String template, Object... args) {
    String localTemplate = String.valueOf(template); // null -> "null"

    // start substituting the arguments into the '%s' placeholders
    StringBuilder builder = new StringBuilder(localTemplate.length() + 16 * args.length);
    int templateStart = 0;
    int i = 0;
    while (i < args.length) {
      int placeholderStart = localTemplate.indexOf("%s", templateStart);
      if (placeholderStart == -1) {
        break;
      }
      builder.append(localTemplate, templateStart, placeholderStart);
      builder.append(args[i++]);
      templateStart = placeholderStart + 2;
    }
    builder.append(localTemplate, templateStart, localTemplate.length());
